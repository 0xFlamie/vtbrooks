...
def detect_events(sym, result, prev):
    ...
    try:
        ...
        pass
    # BTC联动形态(2026-08-23挖掘): 缩量超卖+BTC大跌→24h涨74%; 强势+挤压+BTC大涨→24h偏空66%
    try:
        b7 = fetch_klines("BTCUSDT", "4h", 50)["close"]
        btc7d = float((b7.iloc[-1] / b7.iloc[-43] - 1) * 100) if len(b7) >= 43 else None
        cur["btc7d"] = btc7d
    ...
    if prev:
        ...
        if cur.get("sig_quietbottom") and not prev.get("sig_quietbottom"):
...
SYS_4H = ("你是只做ETH的专业加密交易员(BTC为联动参照), 看大周期: 根据4h简报判断未来4-12小时的方向和最大涨跌幅。"
          "你的交易风格: 观望只在证据真正冲突/混乱时用, 别拿它当默认答案——有边际倾向就给方向+对应的低置信, 市场大部分时间都有倾向; "
          ...
          "例: \"1918两次冲不上去还带放量, 说明有人在这位置持续出货, 多单别在1916上方追, 等回踩1905再看; 站稳1918则转多\"。"
          "只挑最有信息量的2-4条把话说透, 没话说别硬凑。用大白话, 保留关键数字, 不用术语缩写, 每条不超过40字。"
          "magnitude 与方向无关也要给。")

SYS_15M = ("你是只做ETH的专业加密交易员(BTC为联动参照), 看短线: 根据15分钟简报判断未来1-2小时的方向(入场时机)。"
           "你的交易风格: 观望只在证据真正冲突时用, 别当默认答案; 有边际倾向就给方向+低置信。不被投票分布锚定(投票只是参考); 每次判断都带失效条件。"
           ...
           "例: \"缩量反弹到布林上轨, 没人愿意在高位接, 这种反弹是用来出的不是追的, 站稳上轨再转多, 站不稳就继续看跌\"。"
