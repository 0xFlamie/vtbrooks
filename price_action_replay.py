...
from brooks_evidence import confirmed_swings, inspect_evidence
...
def inspect_bar(frame, index, timeframe):
    ...
    evidence = inspect_evidence(window)
    ...
    return {"time": frame.index[index].isoformat(), "timeframe": timeframe, "price": float(last.close),
            "structure": evidence["structure"], "quality": evidence["quality"],
            "swings": evidence["swings"], "support": low, "resistance": high,
