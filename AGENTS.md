...
- **交易员观点段（2026-08-20）**：卡片顶部 4 行——🎯观点（方向+置信+幅度档）→ 📌计划（仓位%+入场+止损距离%+目标+RR，仓位=幅度档×共振×置信修正封顶 50%）→ 🚫失效（结构位/入场窗口失效位，认错翻边）→ 🔁上次观点（维持/翻转+新证据）。全部规则拼装，不引入 AI 编造
...
- 新增：`tests/test_journal.py`（6 用例）+ `tests/test_analyst.py`（11 用例，unittest，mock 网络）
- 判对率口径变化：观望不再计入判对率分母，只统计执行判例——新口径下判对率会下降，这是诚实的代价
- 新增：卡片顶部交易员观点段（`build_analyst_block`，🎯观点/📌计划/🚫失效/🔁上次，仓位规则表 `position_size` 封顶 50%）；每日复盘信 `daily_review`（北京 21:00 UTC 13:00 定时，`last_review_day` 状态去重）
- 注意：观点段图标避开了 `format_layers` return 前板块标题空行后处理的 `heads` 元组（📋 在 heads 里会触发强制空行，故用 📌）
